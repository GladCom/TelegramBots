...
public class BotUpdateHandler : IUpdateHandler
{
  ...
  private static UserScenarioRepository? userScenarioRepository;
  ...
  public async Task HandleUpdateAsync(ITelegramBotClient botClient, Update update, CancellationToken cancellationToken)
  {
    ...
    switch (BotHelper.GetMessage(update))
    {
      case BotChatCommands.Start:
      {
        ...
        userScenarioRepository?.Remove(userId);
        break;
      }
      ...
      {
        var info = BotDbContext.Instance.UserInfos.FirstOrDefault(u => u.UserId == userId);
...
#pragma warning restore CA1031
    ...
    if (userScenario == null && userScenarioRepository.TryGet(userId, out var _userScenario))
      userScenario = _userScenario;
    else
      userScenarioRepository.AddOrReplace(userScenario);

    if (userScenario != null && !(await userScenario.Run(botClient, update, userId)))
      userScenarioRepository.Remove(userScenario);
  }
  ...
  {
    userScenarioRepository = new UserScenarioRepository();
  }
}